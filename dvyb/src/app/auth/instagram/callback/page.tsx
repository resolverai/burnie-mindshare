  ...
  useEffect(() => {
    const handleCallback = async () => {
      ...
      try {
        ...
        if (error) {
          throw new Error(errorDescription || error || 'Instagram authorization failed')
        }
        
        if (!code || !state) {
          throw new Error('Missing code or state from Instagram callback')
        }
        
        console.log('Processing Instagram callback...', { code: code.substring(0, 10) + '...', state })
        
        // Call the backend to complete Instagram connection
        const response = await socialConnectionsApi.handleInstagramConnectCallback(code, state)
        
        if (!response.success) {
          throw new Error(String(('error' in response ? response.error : undefined) || 'Callback failed'))
        }
        
        setStatus('Instagram connected! Closing window...')
        ...
        setIsError(true)
        setStatus(error?.message || 'Instagram connection failed')
