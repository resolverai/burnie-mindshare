...
const seasons = [
  { id: 'spring', name: 'Spring', icon: '🌸', colors: ['Pastels', 'Fresh greens', 'Light blues', 'Soft pinks'] },
  { id: 'summer', name: 'Summer', icon: '☀️', colors: ['Bright whites', 'Ocean blues', 'Sunny yellows', 'Coral'] },
  { id: 'fall', name: 'Fall/Autumn', icon: '🍂', colors: ['Earth tones', 'Burgundy', 'Mustard', 'Forest green'] },
  { id: 'winter', name: 'Winter', icon: '❄️', colors: ['Deep jewel tones', 'Navy', 'Emerald', 'Rich reds'] }
]

const campaignStyles = [
  'Editorial', 'Lifestyle', 'Minimalist', 'Dramatic', 'Cozy', 'Fresh & Clean', 
  'Festive', 'Romantic', 'Bold & Vibrant', 'Natural & Organic'
]

const occasions = [
  'Holiday Shopping', 'Back to School', 'Valentine\'s Day', 'Summer Vacation', 
  'New Year', 'Beach Season', 'Festival Season', 'Wedding Season'
]

export default function SeasonalCampaignPage() {
  ...
  const [sections, setSections] = useState<CollapsibleSection[]>([
    ...
    { id: 'season', title: '3. Select Season', isOpen: false },
    { id: 'style', title: '4. Campaign Style', isOpen: false },
    { id: 'occasions', title: '5. Target Occasions', isOpen: false },
    { id: 'variations', title: '6. Number of Variations', isOpen: false },
    { id: 'options', title: '7. Additional Options', isOpen: false }
  ])
  ...
  const toggleOccasion = (occasion: string) => {
    const newOccasions = new Set(selectedOccasions)
    if (newOccasions.has(occasion)) {
  ...
  const handleGenerate = async () => {
    ...
    if (!selectedStyle) {
      alert('Please select a campaign style')
      return
    }
    ...
    try {
      ...
      const workflowInputs = {
        ...
        selectedSeason: selectedSeasonObj?.name || selectedSeason,
        selectedStyle,
      ...
      const unifiedRequest = {
        ...
        content_type: 'image',
        industry: 'Fashion',
        workflow_type: 'Seasonal Campaign',
      ...
      console.log('🚀 Starting unified generation for Seasonal Campaign workflow')
            ...
            <div className="space-y-3">
              ...
              <div className="bg-gray-800/50 backdrop-blur-sm rounded-lg border border-gray-700/50">
                <button
                  ...
                  className="w-full p-4 flex items-center justify-between text-left hover:bg-gray-700/30 transition-colors rounded-lg"
                >
                  <h2 className="text-base font-bold text-white">3. Select Season</h2>
                  <svg
                    className={'w-5 h-5 text-gray-400 transition-transform ' + (sections.find(s => s.id === 'season')?.isOpen ? 'rotate-180' : '')}
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
                  >
                ...
                {sections.find(s => s.id === 'season')?.isOpen && (
                  <div className="p-4 border-t border-gray-700">
                    <div className="grid grid-cols-2 gap-3">
                      {seasons.map((season) => (
                        ...
                        >
                          ...
                          <div className="text-sm font-bold mb-2">{season.name}</div>
              ...
              <div className="bg-gray-800/50 backdrop-blur-sm rounded-lg border border-gray-700/50">
                <button
                  ...
                  className="w-full p-4 flex items-center justify-between text-left hover:bg-gray-700/30 transition-colors rounded-lg"
                >
                  <h2 className="text-base font-bold text-white">4. Campaign Style</h2>
                  <svg
                    className={'w-5 h-5 text-gray-400 transition-transform ' + (sections.find(s => s.id === 'style')?.isOpen ? 'rotate-180' : '')}
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
                  >
                ...
                {sections.find(s => s.id === 'style')?.isOpen && (
                  <div className="p-4 border-t border-gray-700">
              ...
              <div className="bg-gray-800/50 backdrop-blur-sm rounded-lg border border-gray-700/50">
                ...
                >
                  <h2 className="text-base font-bold text-white">
                    5. Target Occasions {selectedOccasions.size > 0 && <span className="text-green-400">({selectedOccasions.size})</span>}
                  </h2>
                  <svg
                    className={'w-5 h-5 text-gray-400 transition-transform ' + (sections.find(s => s.id === 'occasions')?.isOpen ? 'rotate-180' : '')}
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
                  >
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      {occasions.map((occasion) => (
                        <button
                          ...
                          className={'px-3 py-2 rounded-lg text-sm font-medium transition-all ' +
                            (selectedOccasions.has(occasion)
              ...
              <div className="bg-gray-800/50 backdrop-blur-sm rounded-lg border border-gray-700/50">
                <button
                  ...
                  className="w-full p-4 flex items-center justify-between text-left hover:bg-gray-700/30 transition-colors rounded-lg"
                >
                  <h2 className="text-base font-bold text-white">6. Number of Variations</h2>
                  <svg
                ...
                {sections.find(s => s.id === 'variations')?.isOpen && (
                  <div className="p-4 border-t border-gray-700">
                    ...
                    <p className="text-xs text-gray-400 mt-2">
                      Generate 2-5 campaign variations
                    </p>
                  </div>
                )}
              </div>
              ...
              <div className="bg-gray-800/50 backdrop-blur-sm rounded-lg border border-gray-700/50">
                <button
                  ...
                  className="w-full p-4 flex items-center justify-between text-left hover:bg-gray-700/30 transition-colors rounded-lg"
                >
                  <h2 className="text-base font-bold text-white">7. Additional Options</h2>
                  <svg
                  ...
                  <div className="p-4 border-t border-gray-700 space-y-3">
                    ...
                    <div>
                      ...
                      <textarea
                        ...
                        placeholder="e.g., Focus on outdoor settings, use natural lighting..."
                        rows={3}
                        className="w-full bg-gray-700/50 border border-gray-600 rounded-lg px-3 py-2 text-sm text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500"
                      />
                    </div>
              ...
              <button
                ...
                disabled={generationState === 'generating' || productImages.length === 0 || !productCategory || !selectedSeason || !selectedStyle}
