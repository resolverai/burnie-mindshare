  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const promptRequest = {
        ...
        content_type: 'image',
        workflow_type: 'presentation',
        target_platform: 'presentation',
        theme: presentationType + ' for ' + clientName,
        user_prompt: 'Create ' + slideType + ' slide for ' + presentationType + ' for ' + clientName + '. Theme: ' + visualTheme + '. Focus: ' + contentFocus + '. ' + additionalInstructions,
      ...
      <div className={'flex-1 flex flex-col overflow-hidden transition-all duration-300 ' + (sidebarExpanded ? 'ml-64' : 'ml-20')}>
        <header className="h-16 bg-gray-900/50 backdrop-blur-sm border-b border-gray-800 flex items-center px-6 flex-shrink-0">
          <div className="flex items-center space-x-4">
            ...
            <PresentationChartLineIcon className="w-6 h-6 text-amber-500" />
            <h1 className="text-xl font-semibold text-white">Presentation & Pitch Decks</h1>
          </div>
        </header>
                ...
                {expandedSections.basic && (
                  <div className="mt-4 space-y-4 p-4 bg-gray-800/50 rounded-lg">
                    <div>
                      ...
                      <input type="text" value={clientName} onChange={(e) => setClientName(e.target.value)} placeholder="Enter client name" className="w-full px-4 py-2 bg-gray-700 border border-gray-600 rounded-lg text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-amber-500" />
                    </div>

                    <div>
                      <label className="block text-sm font-medium text-gray-300 mb-2">Presentation Type</label>
                      <select value={presentationType} onChange={(e) => setPresentationType(e.target.value)} className="w-full px-4 py-2 bg-gray-700 border border-gray-600 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-amber-500">
                        <option value="pitch-deck">Pitch Deck</option>
                        <option value="sales">Sales Presentation</option>
                        <option value="corporate">Corporate Deck</option>
                    ...
                    <div>
                      ...
                      <select value={slideType} onChange={(e) => setSlideType(e.target.value)} className="w-full px-4 py-2 bg-gray-700 border border-gray-600 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-amber-500">
                        ...
                        <option value="closing">Closing/CTA Slide</option>
                      </select>
                    </div>

                    <div>
                      <label className="block text-sm font-medium text-gray-300 mb-2">Content Focus</label>
                      <input type="text" value={contentFocus} onChange={(e) => setContentFocus(e.target.value)} placeholder="e.g., Growth metrics, Product overview..." className="w-full px-4 py-2 bg-gray-700 border border-gray-600 rounded-lg text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-amber-500" />
                    </div>
                  </div>
                )}
              </div>
                ...
                {expandedSections.advanced && (
                  <div className="mt-4 space-y-4 p-4 bg-gray-800/50 rounded-lg">
                    <div>
                      ...
                      <select value={visualTheme} onChange={(e) => setVisualTheme(e.target.value)} className="w-full px-4 py-2 bg-gray-700 border border-gray-600 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-amber-500">
                    ...
                    <div>
                      ...
                      <textarea value={additionalInstructions} onChange={(e) => setAdditionalInstructions(e.target.value)} placeholder="Specify layout, colors, or specific elements..." rows={4} className="w-full px-4 py-2 bg-gray-700 border border-gray-600 rounded-lg text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-amber-500 resize-none" />
                    </div>
                  </div>
                )}
              </div>
            ...
            <div className="w-full max-w-2xl">
              {isGenerating ? (
                <div className="flex flex-col items-center justify-center space-y-4">
                  ...
                  <p className="text-gray-400 text-lg">Creating presentation slides...</p>
                </div>
              ) : generatedImages.length > 0 ? (
                <div className="space-y-4">
                  <h3 className="text-lg font-semibold text-white mb-4">Generated Slides</h3>
                  <div className="grid grid-cols-1 gap-4">
                    {generatedImages.map((url, idx) => (
                      <div key={idx} className="relative group">
                        <img src={url} alt={'Slide ' + (idx + 1)} className="w-full rounded-lg border border-gray-700 group-hover:border-amber-500 transition-colors" />
                        <div className="absolute top-2 right-2 bg-gray-900/80 px-2 py-1 rounded text-xs text-white">Slide {idx + 1}</div>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div className="flex flex-col items-center justify-center text-center space-y-4 py-12">
                  <div className="w-24 h-24 bg-gray-800 rounded-full flex items-center justify-center">
                    <PresentationChartLineIcon className="w-12 h-12 text-gray-600" />
                  </div>
                  <p className="text-gray-400 text-lg">No slides generated yet</p>
                </div>
              )}
            </div>
          </div>
