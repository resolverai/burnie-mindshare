...
import { ChevronDownIcon, ChevronUpIcon, SparklesIcon, SwatchIcon } from '@heroicons/react/24/outline'
...
export default function BrandIdentityPage() {
  ...
  const [designElement, setDesignElement] = useState('logo')
  ...
  const handleGenerate = async () => {
    if (!clientName.trim() || !industryVertical.trim()) {
      alert('Please enter client name and industry')
      return
    }
    ...
    try {
      ...
      const promptRequest = {
        ...
        target_platform: 'design',
        theme: 'Brand identity for ' + clientName + ' in ' + industryVertical,
        user_prompt: 'Create ' + designElement + ' concept for ' + clientName + ', a ' + industryVertical + ' business. Brand personality: ' + brandPersonality + '. Color mood: ' + colorMood + '. ' + additionalInstructions,
      ...
      <div className={'flex-1 flex flex-col overflow-hidden transition-all duration-300 ' + (sidebarExpanded ? 'ml-64' : 'ml-20')}>
        <header className="h-16 bg-gray-900/50 backdrop-blur-sm border-b border-gray-800 flex items-center px-6 flex-shrink-0">
          <div className="flex items-center space-x-4">
            ...
            <h1 className="text-xl font-semibold text-white">Client Brand Identity</h1>
          </div>
        </header>
            ...
            <div className="max-w-2xl">
              <div className="mb-4">
                ...
                {expandedSections.basic && (
                  <div className="mt-4 space-y-4 p-4 bg-gray-800/50 rounded-lg">
                    ...
                    <div>
                      <label className="block text-sm font-medium text-gray-300 mb-2">Design Element</label>
                      <select value={designElement} onChange={(e) => setDesignElement(e.target.value)} className="w-full px-4 py-2 bg-gray-700 border border-gray-600 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-indigo-500">
                        <option value="logo">Logo Concept</option>
                        <option value="brandmark">Brandmark/Icon</option>
                        <option value="color-palette">Color Palette</option>
                        <option value="visual-identity">Full Visual Identity</option>
                        <option value="brand-guidelines">Brand Guidelines Layout</option>
                      </select>
                    </div>

                    <div>
                      <label className="block text-sm font-medium text-gray-300 mb-2">Brand Personality</label>
                      <select value={brandPersonality} onChange={(e) => setBrandPersonality(e.target.value)} className="w-full px-4 py-2 bg-gray-700 border border-gray-600 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-indigo-500">
              ...
              <div className="mb-4">
                ...
                {expandedSections.advanced && (
                  <div className="mt-4 space-y-4 p-4 bg-gray-800/50 rounded-lg">
                    <div>
                      <label className="block text-sm font-medium text-gray-300 mb-2">Color Mood</label>
                      <select value={colorMood} onChange={(e) => setColorMood(e.target.value)} className="w-full px-4 py-2 bg-gray-700 border border-gray-600 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-indigo-500">
                        ...
                        <option value="monochrome">Monochrome & Sophisticated</option>
                    ...
                    <div>
                      ...
                      <textarea
                        ...
                        placeholder="Describe specific design preferences, symbols, themes, or avoid elements..."
                        rows={4}
                        className="w-full px-4 py-2 bg-gray-700 border border-gray-600 rounded-lg text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-indigo-500 resize-none"
                      />
                    </div>
                  </div>
                )}
              ...
              <div className="mb-6">
                ...
                {expandedSections.options && (
                  <div className="mt-4 space-y-4 p-4 bg-gray-800/50 rounded-lg">
                    <div>
                      <label className="block text-sm font-medium text-gray-300 mb-2">Number of Concepts ({numVariations})</label>
              ...
              <button
                ...
                disabled={isGenerating || !clientName.trim() || !industryVertical.trim()}
                className="w-full py-4 bg-gradient-to-r from-indigo-500 to-purple-500 hover:from-indigo-600 hover:to-purple-600 disabled:from-gray-600 disabled:to-gray-700 text-white font-semibold rounded-lg transition-all duration-200 flex items-center justify-center space-x-2"
              >
                {isGenerating ? (
                  <>
            ...
            <div className="w-full max-w-2xl">
              {isGenerating ? (
                <div className="flex flex-col items-center justify-center space-y-4">
                  ...
                  <p className="text-gray-400 text-lg">Creating brand concepts...</p>
                </div>
              ) : generatedImages.length > 0 ? (
                <div className="space-y-4">
                  <h3 className="text-lg font-semibold text-white mb-4">Generated Concepts</h3>
                  <div className="grid grid-cols-1 gap-4">
                    {generatedImages.map((url, idx) => (
                      <div key={idx} className="relative group">
                        <img src={url} alt={'Concept ' + (idx + 1)} className="w-full rounded-lg border border-gray-700 group-hover:border-indigo-500 transition-colors" />
                        <div className="absolute top-2 right-2 bg-gray-900/80 px-2 py-1 rounded text-xs text-white">Concept {idx + 1}</div>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div className="flex flex-col items-center justify-center text-center space-y-4 py-12">
                  <div className="w-24 h-24 bg-gray-800 rounded-full flex items-center justify-center">
                    <SwatchIcon className="w-12 h-12 text-gray-600" />
                  </div>
                  <p className="text-gray-400 text-lg">No concepts generated yet</p>
                </div>
              )}
            </div>
          </div>
