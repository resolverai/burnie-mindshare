...
export default function ProductShowcasePage() {
  ...
  const [focusPoint, setFocusPoint] = useState('features')
  const [photographyStyle, setPhotographyStyle] = useState('professional')
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const promptRequest = {
        ...
        theme: 'Product showcase: ' + productName,
        user_prompt: 'Create professional product photography for ' + productName + ' (' + productType + ' product). Focus on: ' + focusPoint + '. Style: ' + photographyStyle + '. Background: ' + backgroundColor + '. ' + additionalInstructions,
      ...
      <div className={'flex-1 flex flex-col overflow-hidden transition-all duration-300 ' + (sidebarExpanded ? 'ml-64' : 'ml-20')}>
        <header className="h-16 bg-gray-900/50 backdrop-blur-sm border-b border-gray-800 flex items-center px-6 flex-shrink-0">
          <div className="flex items-center space-x-4">
            ...
            <h1 className="text-xl font-semibold text-white">Product/Service Showcase</h1>
          </div>
        </header>
            ...
            <div className="max-w-2xl">
                  ...
                  <div className="mt-4 space-y-4 p-4 bg-gray-800/50 rounded-lg">
                    <div>
                      ...
                      <input
                        ...
                        placeholder="e.g., iPhone 16, Marketing Strategy, Coaching Program..."
                    ...
                    <div>
                      ...
                      <select value={productType} onChange={(e) => setProductType(e.target.value)} className="w-full px-4 py-2 bg-gray-700 border border-gray-600 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-green-500">
                    ...
                    <div>
                      <label className="block text-sm font-medium text-gray-300 mb-2">Focus Point</label>
                      <select value={focusPoint} onChange={(e) => setFocusPoint(e.target.value)} className="w-full px-4 py-2 bg-gray-700 border border-gray-600 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-green-500">
                        <option value="features">Key Features</option>
                        <option value="benefits">Benefits & Value</option>
                        <option value="aesthetics">Aesthetics & Design</option>
                        <option value="use-case">Use Case / In Action</option>
                        <option value="comparison">vs Competition</option>
                      </select>
                    </div>
                  </div>
                )}
              ...
              <div className="mb-4">
                ...
                {expandedSections.advanced && (
                  <div className="mt-4 space-y-4 p-4 bg-gray-800/50 rounded-lg">
                    <div>
                      <label className="block text-sm font-medium text-gray-300 mb-2">Photography Style</label>
                      <select value={photographyStyle} onChange={(e) => setPhotographyStyle(e.target.value)} className="w-full px-4 py-2 bg-gray-700 border border-gray-600 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-green-500">
                        <option value="professional">Professional Studio</option>
                        <option value="lifestyle">Lifestyle / Real-world</option>
                    ...
                    <div>
                      ...
                      <select value={backgroundColor} onChange={(e) => setBackgroundColor(e.target.value)} className="w-full px-4 py-2 bg-gray-700 border border-gray-600 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-green-500">
                        <option value="white">White</option>
                        <option value="black">Black</option>
                        <option value="gradient">Gradient</option>
                        <option value="contextual">Contextual Scene</option>
                        <option value="transparent">Transparent/Isolated</option>
                      </select>
                    </div>
                    ...
                    <div>
                      ...
                      <textarea
                        ...
                        placeholder="Describe specific angles, features to highlight, mood..."
                        rows={4}
                        className="w-full px-4 py-2 bg-gray-700 border border-gray-600 rounded-lg text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-green-500 resize-none"
                      />
                    </div>
                  </div>
                )}
                ...
                {expandedSections.options && (
                  <div className="mt-4 space-y-4 p-4 bg-gray-800/50 rounded-lg">
                    ...
                    <div className="flex items-center space-x-3">
                      <input type="checkbox" id="includeLogo" checked={includeLogo} onChange={(e) => setIncludeLogo(e.target.checked)} className="w-4 h-4 bg-gray-700 border-gray-600 rounded focus:ring-2 focus:ring-green-500" />
              ...
              >
                {isGenerating ? (
                  ...
                  <>
                    ...
                    <span>Generate Product Showcase</span>
                  </>
                )}
              </button>
            </div>
                  ...
                  <div className="grid grid-cols-1 gap-4">
                    {generatedImages.map((url, idx) => (
                      <div key={idx} className="relative group">
                        <img src={url} alt={'Generated ' + (idx + 1)} className="w-full rounded-lg border border-gray-700 group-hover:border-green-500 transition-colors" />
