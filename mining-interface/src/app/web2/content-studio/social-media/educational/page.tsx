...
import { ChevronDownIcon, ChevronUpIcon, SparklesIcon, AcademicCapIcon } from '@heroicons/react/24/outline'
...
export default function EducationalPage() {
  ...
  const [contentFormat, setContentFormat] = useState('step-by-step')
  const [targetAudience, setTargetAudience] = useState('beginners')
  const [visualStyle, setVisualStyle] = useState('infographic')
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const promptRequest = {
        ...
        theme: 'Educational content: ' + contentTopic,
        user_prompt: 'Create ' + contentFormat + ' educational content about ' + contentTopic + ' for ' + targetAudience + '. Visual style: ' + visualStyle + '. Number of steps: ' + numSteps + '. ' + additionalInstructions,
      ...
      <div className={'flex-1 flex flex-col overflow-hidden transition-all duration-300 ' + (sidebarExpanded ? 'ml-64' : 'ml-20')}>
        <header className="h-16 bg-gray-900/50 backdrop-blur-sm border-b border-gray-800 flex items-center px-6 flex-shrink-0">
          <div className="flex items-center space-x-4">
            ...
            <h1 className="text-xl font-semibold text-white">Educational / How-To Content</h1>
          </div>
        </header>
                  ...
                  <div className="mt-4 space-y-4 p-4 bg-gray-800/50 rounded-lg">
                    <div>
                      ...
                      <input
                        ...
                        placeholder="e.g., How to use AI tools, 5 tips for productivity..."
                    ...
                    <div>
                      ...
                      >
                        ...
                        <option value="comparison">Comparison / Before-After</option>
                    ...
                    <div>
                      <label className="block text-sm font-medium text-gray-300 mb-2">Number of Steps ({numSteps})</label>
                      <input
                        type="range"
                        min="3"
                        max="10"
              ...
              {isGenerating ? (
                    ...
                    {generatedImages.map((url, idx) => (
                      <div key={idx} className="relative group">
                        <img
                          ...
                          className="w-full rounded-lg border border-gray-700 group-hover:border-blue-500 transition-colors"
                        />
                ...
                <div className="flex flex-col items-center justify-center text-center space-y-4 py-12">
                  <div className="w-24 h-24 bg-gray-800 rounded-full flex items-center justify-center">
                    <AcademicCapIcon className="w-12 h-12 text-gray-600" />
                  </div>
                  <p className="text-gray-400 text-lg">No content generated yet</p>
                  <p className="text-gray-500 text-sm max-w-md">
                    Fill in the form and click "Generate Educational Content" to create step-by-step guides
                  </p>
                </div>
              )}
            </div>
