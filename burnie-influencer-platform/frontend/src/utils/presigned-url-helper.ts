/**
 * Pre-signed URL Helper for Yapper Dashboard
...
class PresignedUrlManager {
  ...
  /**
   * Get pre-signed URL from TypeScript backend
   */
  private async fetchPresignedUrlFromBackend(contentId: string): Promise<PresignedUrlResponse> {
    try {
      const response = await fetch(`${this.baseUrl}/api/marketplace/content/${contentId}/presigned-url`, {
        method: 'POST',
        headers: {
  ...
  /**
   * Get pre-signed URL directly from Python backend (fallback)
   */
   ...
   */
  async getPresignedUrl(originalUrl: string, contentId?: string): Promise<string> {
    try {
      ...
      // Try to get pre-signed URL via TypeScript backend if contentId is provided
      let result: PresignedUrlResponse;
      if (contentId) {
        result = await this.fetchPresignedUrlFromBackend(contentId);
        // Fallback to direct Python backend call if TypeScript backend fails
        if (!result.success) {
          result = await this.fetchPresignedUrlDirect(s3Key);
        }
      } else {
   ...
   */
  async preloadUrls(urlsWithIds: Array<{url: string, contentId?: string}>): Promise<Map<string, string>> {
    const urlMap = new Map<string, string>();
    
    const promises = urlsWithIds.map(async (item) => {
      const presignedUrl = await this.getPresignedUrl(item.url, item.contentId);
      urlMap.set(item.url, presignedUrl);
      return { original: item.url, presigned: presignedUrl };
    });
 ...
 */
export const usePresignedUrl = (originalUrl: string | null, contentId?: string) => {
  ...
  React.useEffect(() => {
    ...
    presignedUrlManager.getPresignedUrl(originalUrl, contentId)
      .then((url) => {
        setPresignedUrl(url);
        setLoading(false);
      })
