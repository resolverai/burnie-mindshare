...
Assets Structure:
    raw_assets/
    ├── master_metadata.json    # Generation params, clip info (identifies generator)
    ├── video_plan.json         # Grok-generated video plan
    ├── input_context.txt       # Original script text
    ├── transcriptions.json     # Word timestamps for captions (if captions were used)
    ├── effect_analysis.json    # Applied effects (political_video_generator only)
    ├── videos/                 # Raw video clips (SILENT_IMAGE, B_ROLL, AI_VIDEO)
    │   └── clip_*_raw.mp4
    ├── images/                 # Generated images (starting frames for B_ROLL, influencer clips)
    │   └── clip_*.png
    ├── voiceovers/             # Voiceover audio files
    │   └── voiceover_clip_*.mp3
    ├── music/                  # Background music files (first one loops throughout)
    │   └── music_Music_A.mp3   # Uses only first music group, loops for entire video
    └── research_clips/         # Research clips to insert after specific clips
        ├── research_after_clip_{n}.mp4       # Research video
        ├── research_vo_after_clip_{n}.mp3    # Research voiceover
        └── research_after_clip_{n}_info.json # Research metadata
...
def apply_speed_and_music(video_path: str, output_path: str, speed: float = 1.0, 
    ...
    try:
        print(f"\n{'='*60}")
        print(f"🚀 APPLYING SPEED AND MUSIC")
        print(f"{'='*60}")
        ...
        if music_path and os.path.exists(music_path):
            ...
            print(f"  🎵 Music will start at {music_start_time:.2f}s (skipping Clip 0)")
        ...
        if final_audio is not None:
            ...
            print(f"  ✅ Final audio duration: {final_audio.duration:.2f}s (video: {new_duration:.2f}s, buffer: {END_BUFFER*1000:.0f}ms)")
        ...
        video.close()
        print(f"  ✅ Speed and music applied successfully")
        return output_path
        
    except Exception as e:
        print(f"  ❌ Failed to apply speed/music: {e}")
        import traceback
        traceback.print_exc()
        return None
...
def stitch_clips_with_separate_audio(
    ...
    """
    print(f"\n{'='*60}")
    print(f"🎬 STITCHING WITH SEPARATE AUDIO HANDLING")
    print(f"{'='*60}")
    ...
    if clip0_voiceover:
        print(f"  🎤 Clip 0 voiceover: {os.path.basename(clip0_voiceover)}")
...
def load_research_clips(assets_folder: str) -> List[Dict]:
    ...
    for info_file in sorted(info_files):
        try:
            ...
            print(f"  ⚠️ Error loading research clip info: {e}")
            continue
...
def regenerate_from_raw(assets_folder: str, output_path: Optional[str] = None,
    ...
    """
    print(f"\n{'='*60}")
    print(f"🔄 REGENERATING FROM RAW ASSETS")
    print(f"{'='*60}")
    print(f"  Assets folder: {assets_folder}")
    
    raw_assets_dir = os.path.join(assets_folder, "raw_assets")
    if not os.path.exists(raw_assets_dir):
        print(f"  ❌ Raw assets folder not found: {raw_assets_dir}")
        return None
    ...
    print(f"  🎬 Generator: {generator_display}")
    
    # Load master metadata
    metadata = load_master_metadata(assets_folder)
    if not metadata:
        print(f"  ⚠️ No master_metadata.json found, using file discovery")
    else:
        print(f"  ✅ Loaded master metadata")
    ...
    if video_plan:
        print(f"  ✅ Loaded video plan ({len(video_plan.get('clips', []))} clips)")
    
    effect_analysis = load_effect_analysis(assets_folder)
    if effect_analysis:
        print(f"  ✅ Loaded effect analysis ({len(effect_analysis)} clips)")
    
    transcriptions = {}
    if use_saved_transcriptions:
        transcriptions = load_transcriptions(assets_folder)
        if transcriptions:
            print(f"  ✅ Loaded transcriptions ({len(transcriptions)} clips)")
    ...
    if not video_files:
        print(f"  ❌ No raw video clips found in {raw_videos_dir}")
    ...
    if research_clips:
        ...
        for research_clip in research_clips:
            ...
            try:
                ...
                print(f"  ⚠️ Clip {insert_after_clip_num} not found, skipping research clip")
                continue
    ...
    if not include_music:
        music_path = None
        print(f"  🔇 No background music (--no-music flag)")
    elif custom_music_path and os.path.exists(custom_music_path):
        music_path = custom_music_path
        print(f"  🎵 Using CUSTOM music: {os.path.basename(music_path)} (will loop throughout)")
    else:
    ...
    if video_paths and clip_numbers and clip_numbers[0] == 0:
        try:
            ...
            temp_clip.close()
            print(f"  📏 Clip 0 duration: {clip_0_duration:.2f}s (music will start after this)")
        except Exception as e:
            print(f"  ⚠️ Could not get clip 0 duration: {e}, using default 0")
            clip_0_duration = 0.0
    
    # If speed != 1.0, stitch WITHOUT music first, then apply speed, then add music
    if speed != 1.0:
        print(f"\n  🚀 Speed mode: {speed}x - will stitch without music, then speed up, then add music")
    ...
    if result and captions:
        print(f"\n{'='*60}")
        print(f"📝 APPLYING CAPTIONS: {captions}")
        print(f"{'='*60}")
        try:
            ...
            if combo:
                ...
                print(f"  🎬 Captions will start at {caption_start_time:.2f}s (after Clip 0)")
                ...
                if os.path.exists(captioned_path):
                    ...
                    print(f"  ✅ Captions applied successfully!")
                    print(f"  Output: {result}")
                else:
                    print(f"  ❌ Captioned file was not created: {captioned_path}")
            else:
                print(f"  ❌ Unknown caption style: {captions}")
                print(f"     Valid styles include: karaoke_red, karaoke_blue, boxed_white, etc.")
        except ImportError as e:
            print(f"  ❌ Failed to import video_captions module: {e}")
        except Exception as e:
...
def restitch_only(assets_folder: str, output_path: Optional[str] = None,
    ...
    """
    print(f"\n{'='*60}")
    print(f"🔗 RE-STITCHING COMPLETE CLIPS")
    print(f"{'='*60}")
    print(f"  Assets folder: {assets_folder}")
    ...
    if not clip_folders:
        print(f"  ❌ No clip folders found in {assets_folder}")
        return None
    ...
    if not include_music:
        ...
        music_path = custom_music_path
        print(f"  🎵 Using CUSTOM music: {os.path.basename(music_path)}")
    ...
    if speed != 1.0:
        ...
        print(f"  🔄 Will replace existing music with custom music")
...
def fix_assets(assets_folder: str, output_path: Optional[str] = None, 
    ...
    print(f"\n{'='*60}")
    print(f"📦 FIXING INDIVIDUAL CLIPS")
    print(f"{'='*60}")
    ...
    if not fixed_clips:
        print(f"\n  ❌ No clips were fixed successfully")
        return None
...
Examples:
    ...
    if args.list_assets:
        ...
        if video_plan:
            print(f"\n  📝 Video Plan: {len(video_plan.get('clips', []))} clips defined")
        
        # Check effect analysis
        effects = load_effect_analysis(args.assets_folder)
        if effects:
            print(f"\n  🎨 Effect Analysis: {len(effects)} clips with effects")
        
        # Check transcriptions
        transcriptions = load_transcriptions(args.assets_folder)
        if transcriptions:
            print(f"\n  💬 Transcriptions: {len(transcriptions)} clips with word timestamps")
        ...
        if clip_folders:
            print(f"\n  🎬 Complete Clips: {len(clip_folders)} processed clips")
        ...
        print(f"\n  ✅ Assets sufficient for regeneration: {'YES' if sufficient else 'PARTIAL'}")
        if not sufficient:
            missing = []
            if not has_videos:
    ...
    if custom_music:
        if os.path.exists(custom_music):
            print(f"🎵 Using custom music: {custom_music}")
        else:
            print(f"⚠️ Custom music file not found: {custom_music}")
            custom_music = None
    
    # Handle speed parameter
    speed = args.speed
    if speed != 1.0:
        print(f"🚀 Speed: {speed}x (will speed up visuals + voiceover, then apply music)")
