...
router.get('/hot-campaigns', async (req, res) => {
  try {
    ...
    // Filter out Somnia whitelisted projects - they should NOT be generated by miners
    const somniaExcludedCount = hotCampaigns.filter(c => c.isSomniaWhitelisted).length;
    const nonSomniaHotCampaigns = hotCampaigns.filter(c => !c.isSomniaWhitelisted);

    logger.info(`🚫 Excluded ${somniaExcludedCount} Somnia whitelisted campaign post_types from miner generation`);
    logger.info(`✅ ${nonSomniaHotCampaigns.length} non-Somnia hot campaign post_types available for miners`);
    ...
    // Shuffle all non-Somnia hot campaigns for fair distribution
    const finalHotCampaigns = shuffleArray(nonSomniaHotCampaigns);
