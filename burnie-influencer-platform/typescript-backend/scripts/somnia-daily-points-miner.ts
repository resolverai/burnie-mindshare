...
class SomniaDailyPointsMinerScript {
  ...
  async calculateWeeklyRewards(): Promise<void> {
    console.log('🗓️ Calculating weekly rewards (Tuesday calculation)...');
    ...
    if (eligibleForUptime.length > 0) {
      ...
      for (const miner of eligibleForUptime) {
        ...
        console.log(`💰 Uptime reward: ${miner.walletAddress} (${parseFloat(miner.uptime_percentage).toFixed(2)}% uptime) = ${uptimeRewardPerMiner} $ROAST`);
      }
    }
    ...
    if (eligibleSellers.length > 0) {
      ...
      for (let i = 0; i < eligibleSellers.length; i++) {
        ...
        console.log(`🏆 Rank ${i + 1}: ${miner.walletAddress} - ${parseFloat(miner.total_sales).toFixed(2)} sales = ${topSellerBonus} bonus`);
      }
    }
  ...
  printDryRunSummary(): void {
    console.log('\n' + '='.repeat(200));
    console.log('📊 DRY RUN SUMMARY - MINER POINTS BREAKDOWN');
  ...
  async run(dryRun: boolean = false): Promise<void> {
    try {
      ...
      for (const miner of miners) {
        try {
          console.log(`\n👤 Processing miner: ${miner.walletAddress}`);
          ...
          console.log(`  📅 Calculating since: ${sinceTimestamp.toISOString()} (${daysSinceLastRun} days ago)`);
          
          // Get content and sales data by project
          console.log(`  📊 Fetching content generated by project...`);
          const contentByProject = await this.getContentGeneratedByProject(miner.walletAddress, sinceTimestamp);
          
          console.log(`  📊 Fetching sales data by project...`);
          ...
          if (activeProjectIds.size === 0) {
            console.log(`  ⚠️ No activity found for this miner since ${sinceTimestamp.toISOString()}`);
            continue;
          }
          
          console.log(`  🎯 Miner has activity for ${activeProjectIds.size} projects`);
          ...
          for (const projectId of activeProjectIds) {
            ...
            if (dryRun) {
              ...
              console.log(`  📊 [DRY RUN] Project ${projectId}: Content=${contentCount}, Sold=${salesData.contentSold}, Revenue=${salesData.salesRevenue.toFixed(2)}`);
            } else {
              await this.saveMinerDailyPoints(calculation);
              console.log(`  ✅ Saved record for project ${projectId}`);
            }
            
            totalProjectRecords++;
          }
          
          console.log(`  📊 Total: ${activeProjectIds.size} project records created`);
      ...
      console.log(`\n📊 [${dryRun ? 'DRY RUN' : 'LIVE'}] Created ${totalProjectRecords} project records for ${miners.length} miners`);
      ...
      console.log(`\n🎉 Somnia Miner Points Calculation completed successfully!${dryRun ? ' [DRY RUN MODE]' : ''}`);
